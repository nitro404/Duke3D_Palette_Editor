...
public class PreferredPalettePluginEditorDialog extends JDialog implements ActionListener, KeyListener {
	...
	public PreferredPalettePluginEditorDialog(Frame parent) {
		super(parent, true);
	...
	private void savePreferredPlugins() {
		...
		for(int i=0;i<m_pluginListComboBoxes.size();i++) {
			...
			if(formattedPreferredPluginName == null) {
				PalettePluginManager.instance.removePreferredPluginForFileFormat(m_fileFormatLabels.elementAt(i).getText(), PalettePlugin.class);
			}
			else {
				PalettePluginManager.instance.setPreferredPluginForFileFormat(m_fileFormatLabels.elementAt(i).getText(), formattedPreferredPluginName, PalettePlugin.class);
			}
		}
	}
	...
	private void loadPreferredPlugins() {
		...
		Vector<String> paletteFileFormats = PalettePluginManager.instance.getSupportedAndPreferredPaletteFileFormats();
		
		String fileFormat = null;
		JLabel fileFormatLabel = null;
		JComboBox<String> pluginListComboBox = null;
		JTextField customPluginTextField = null;
		Vector<PalettePlugin> palettePlugins = null;
		String palettePluginNames[] = null;
		int preferredPluginIndex = -1;
		String preferredPluginName = null;
		for(int i=0;i<paletteFileFormats.size();i++) {
			fileFormat = paletteFileFormats.elementAt(i);
			
			palettePlugins = PalettePluginManager.instance.getPalettePluginsForFileFormat(fileFormat);
			if(palettePlugins == null || palettePlugins.size() == 0) { continue; }
			
			preferredPluginIndex = -1;
			preferredPluginName = PalettePluginManager.instance.getPreferredPluginForFileFormat(fileFormat, PalettePlugin.class);
			
			palettePluginNames = new String[palettePlugins.size() + 2];
			palettePluginNames[0] = "None";
			palettePluginNames[palettePluginNames.length - 1] = "Other";
			for(int j=0;j<palettePlugins.size();j++) {
				palettePluginNames[j + 1] = palettePlugins.elementAt(j).getName();
				
				if(palettePlugins.elementAt(j).getName().equalsIgnoreCase(preferredPluginName)) {
