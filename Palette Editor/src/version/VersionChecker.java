...
public class VersionChecker {
	...
	private static boolean checkVersionHelper(boolean verbose) {
		if(SettingsManager.instance.versionFileURL == null) {
			...
			if(verbose) { JOptionPane.showMessageDialog(PaletteEditor.paletteEditorWindow.getFrame(), message, "Invalid Version File URL", JOptionPane.ERROR_MESSAGE); }
		...
		try {
			...
			if(verbose) { JOptionPane.showMessageDialog(PaletteEditor.paletteEditorWindow.getFrame(), message, "Invalid URL", JOptionPane.ERROR_MESSAGE); }
		...
		try {
			...
			PaletteEditor.console.writeLine(message);
			
			if(verbose) { JOptionPane.showMessageDialog(PaletteEditor.paletteEditorWindow.getFrame(), message, "IO Exception", JOptionPane.ERROR_MESSAGE); }
		...
		try {
			...
			if(verbose) { JOptionPane.showMessageDialog(PaletteEditor.paletteEditorWindow.getFrame(), message, "XML Stream Exception", JOptionPane.ERROR_MESSAGE); }
		...
		try {
			switch(Utilities.compareVersions(PaletteEditor.VERSION, version)) {
				case -1:
					SystemConsole.instance.writeLine("A new version of Duke Nukem 3D Palette Editor is available! Download version " + version + " at the following link: \"" + link + "\".");
					
					if(verbose || !SettingsManager.instance.supressUpdates) { JOptionPane.showMessageDialog(PaletteEditor.paletteEditorWindow.getFrame(), "A new version of Duke Nukem 3D Palette Editor is available! Released " + date + ".\nDownload version " + version + " at the following link: \"" + link + "\".", "New Version Available", JOptionPane.INFORMATION_MESSAGE); }
					
					break;
				case 0:
					String message = "Duke Nukem 3D Palette Editor is up to date with version " + PaletteEditor.VERSION + ", released " + date + ".";
					
					SystemConsole.instance.writeLine(message);
					
					if(verbose) { JOptionPane.showMessageDialog(PaletteEditor.paletteEditorWindow.getFrame(), message, "Up to Date", JOptionPane.INFORMATION_MESSAGE); }
					
					break;
				case 1:
					SystemConsole.instance.writeLine("Wow, you're from the future? Awesome. Hope you're enjoying your spiffy version " + PaletteEditor.VERSION + " of the Duke Nukem 3D Palette Editor!");
			...
			if(verbose) { JOptionPane.showMessageDialog(PaletteEditor.paletteEditorWindow.getFrame(), message, "Invalid Version", JOptionPane.ERROR_MESSAGE); }
